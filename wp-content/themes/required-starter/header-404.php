...
<body <?php body_class(); ?> data-ng-app="ngAppDemo">
	...
	<canvas id="spiro" canvasbg >1</canvas>
		...
		if($chkbanner["wpcf-banner-image"][0] === ""){ ?>
			<div id="slider">
				<?php 
				if($chkbanner["wpcf-layer-slider-id"][0] === ""){
					layerslider(1);
				}else{
					layerslider($chkbanner["wpcf-layer-slider-id"][0]);
				} 
				?>
			</div>

		<?php 		
		}elseif (!isset($chkbanner["wpcf-banner-image"][0])) {
			# code...
		} else{ ?>
			<div id="slider" class="bannerimgslide">
				<img class="bannerimg" src="<?php echo $chkbanner["wpcf-banner-image"][0]; ?>" alt="Banner image">
			</div>
		<?php } ?>
