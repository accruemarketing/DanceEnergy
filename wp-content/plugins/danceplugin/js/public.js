...
myApp.controller('findpartner', function($scope, $http, usSpinnerService, getpartners, $sce, getpartner, savepartner, $filter){

    //default options to send to endpoints...
    $scope.partnerid = {'ID': ''};
    $scope.partnersavesid = {};
    $scope.singlepartner = false;
    $scope.searchcontent = true;
    $scope.showsaved = true;
    //start spinner on page load this doesn't work :*(
    usSpinnerService.spin('spinner-1');
    
    $scope.filters = { };
    
    $scope.showallpartner = function showallpartner(){
    
        $scope.showsaved = true;

    };

    $scope.getsavedonly = function getsavedonly(){
        $scope.showsaved = false;
        console.log($filter);
        alert("steady on tiger! this isn't done yet!");
    ...
    $scope.uncheckall = function uncheckall(){
        angular.forEach($scope.findpartnerall, function(value, key){
    ...
    $scope.SaveListing = function SaveListing(){
        usSpinnerService.spin('spinner-1');

       angular.forEach($scope.findpartnerall, function(key, value){
                $scope.partnersavesid[key.ID] = key.selected;
       });

        savepartner.savepartner($scope)
        .success(function(data, status, headers, config){
            usSpinnerService.stop('spinner-1');
        })
        .error(function(data, status, headers, config) {
    ...
    //Delete selected parteners
    $scope.DeleteSelected = function DeleteSelected(){
        usSpinnerService.spin('spinner-1');

        angular.forEach($scope.findpartnerall, function(key, value){ $scope.partnersavesid[key.ID] = false; });


        savepartner.savepartner($scope)
        .success(function(data, status, headers, config){
            getpartners.getpartners().success(function(data, status, headers, config){

                usSpinnerService.stop('spinner-1');
                $scope.findpartnerall = angular.fromJson(data);
    ...
    //load partner info
    $scope.loadmore = function loadmore(ID, user_nicename){
        usSpinnerService.spin('spinner-1');
        event.preventDefault();
        $scope.partnerid.ID = ID;

        getpartner.getpartner($scope)
        .success(function(data, status, headers, config){
        
            usSpinnerService.stop('spinner-1');
            $scope.findpartner = angular.fromJson(data);
            $scope.singlepartner = true;
    ...
    $scope.viewsaved = function getsavedonly(){
        alert('not yet');
    };
    //load all people thaty are look for a partner...
...
Factory's
*/
myApp.factory('getpartners', function($http){
    return {
        getpartners: function() {
            return  $http({
                        method: 'POST', 
                        url: '/wp-admin/admin-ajax.php',
                        params: { 'action': 'get_partners_loop'}
                    });
        }
    };
});
myApp.factory('getpartner', function($http){
    return {
        getpartner: function($scope) {
            return  $http({
                        method: 'POST', 
                        url: '/wp-admin/admin-ajax.php',
                        data: $scope.partnerid,
                        params: { 'action': 'get_partner_loop'}
                    });
        }
    };
});
myApp.factory('savepartner', function($http){
    return {
        savepartner: function($scope) {
            return  $http({
                        method: 'POST', 
                        url: '/wp-admin/admin-ajax.php',
                        data: $scope.partnersavesid,
                        params: { 'action': 'save_partner'}
                    });
        }
    };
});
