...
/****
	*
	*INSTRUCTOR WIDGET CLASS
	*
	**/
class instructors_widget_info extends WP_Widget {


		//Name the widget, here Instructors info will be displayed as widget name, $widget_ops may be an array of value, which may holds the title, description like that.

		function instructors_widget_info () {

			$this->WP_Widget('instructors_widget_info', 'Meet our instructors', $widget_ops );        
		}
		...
		public function form( $instance ) {
			...
			<ul>
				<?php 
				    ...
				    $custom_posts->query("post_type=instructor&posts_per_page=$number&orderby=$orderby");
			...
			<p>MAX Number of instrutors to show: <input name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo esc_attr( $number );?>" /></p>
			<p>Excerpt length: <input name="<?php echo $this->get_field_name( 'excerpt' ); ?>" type="text" value="<?php echo esc_attr( $excerpt );?>" /></p>
			<p>Order instructor by: <br> <select name="<?php echo $this->get_field_name( 'orderby' ); ?>" value="<?php echo esc_attr( $orderby );?>" id="">
				...
				(function ($) {
					...
					$(function () {
						// Place your public-facing JavaScript here
						    jQuery('#instructors_slideshow').bjqs({
			...
			<div id="instructors_slideshow">
				<ul class="bjqs">
					<?php 
						...
						$bio = myTruncate($postmeta['wpcf-instructor-bio'][0], $excerpt);
						$img = $postmeta['wpcf-profile-photo-instructor'][0];
						$specialty = $postmeta['wpcf-instructor-specialty'][0];
						/*
						echo "<pre>";
					...
					?>
					    ...
					    <li>
					    	...
					    	<span class="eight columns">
					    		...
					    		<br/>
					    		<span>Specialty: <?php echo $specialty; ?></span>
					    	</span>
							<span class="twelve columns"><?php echo $bio; ?></span>
					    </li>
					<?php endwhile; ?> 
					<?php  wp_reset_postdata(); ?>
				</ul>
