...
/****
	...
	*
	**/
class faq_widget_info extends WP_Widget {


		//Name the widget, here faq info will be displayed as widget name, $widget_ops may be an array of value, which may holds the title, description like that.

		function faq_widget_info () {

			$this->WP_Widget('faq_widget_info', 'Faq', $widget_ops );        
		}
		...
		public function form( $instance ) {
			...
			<ul>
				<?php 
				    ...
				    $custom_posts->query("post_type=frec-quests&posts_per_page=$number&orderby=$orderby");
			...
			<p>MAX Number of FAQ's to show: <input name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo esc_attr( $number );?>" /></p>
			<p>Excerpt length: <input name="<?php echo $this->get_field_name( 'excerpt' ); ?>" type="text" value="<?php echo esc_attr( $excerpt );?>" /></p>
			<p>Order FAQ's by: <br> <select name="<?php echo $this->get_field_name( 'orderby' ); ?>" value="<?php echo esc_attr( $orderby );?>" id="">
